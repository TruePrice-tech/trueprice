...
{
  "contractor": <string or null - concrete contractor company name. Look at the top of the quote (letterhead), header, footer, signature line, or "from"/"prepared by" sections. Extract the company name even if it's in a logo image. This is REQUIRED whenever a name is visible.>,
...
- Return ONLY the JSON object, nothing else`
    ...
    if (req.headers["x-woogoro-test"] !== "1") captureAnonymizedData("concrete", parsed);
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        await guardedFlywheelBump(redis, "concrete", totalPrice, cityLc, st, { incRealQuote: !!_imageBuf });
      }
    } catch (calErr) {
