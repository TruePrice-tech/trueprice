...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "foundation", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
- Return ONLY the JSON object, nothing else`
    ...
    if (_guard.imageHash) {
      await storeImageCache("foundation", _guard.imageHash, { success: true, source: "claude-haiku", data: parsed });
    }
    ...
    if (req.headers["x-trueprice-test"] !== "1") captureAnonymizedData("foundation", parsed); // fire and forget
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        if (st) {
          ...
          if (cityLc) await bump(`cal:${cityLc}:${st}:foundation`);
          await bump(`cal:metro:${st}:foundation`);
        }
      }
    } catch (calErr) {
