...
- Return ONLY the JSON object, nothing else`
    ...
    if (req.headers["x-trueprice-test"] !== "1") captureAnonymizedData("gutters", parsed);
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        if (st) {
          ...
          if (cityLc) await bump(`cal:${cityLc}:${st}:gutters`);
          await bump(`cal:metro:${st}:gutters`);
        }
      }
    } catch (calErr) {
