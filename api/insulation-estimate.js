...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "insulation", cacheNamespace: "insulation:v6-multiquote-2026-09-11", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
{
  "contractor": <string or null - insulation contractor company name. Look at the top of the quote (letterhead), header, footer, signature line, or "from"/"prepared by" sections. Extract the company name even if it's in a logo image. This is REQUIRED whenever a name is visible.>,
...
- If the document contains MORE THAN ONE distinct insulation contractor's quote (e.g. a comparison sheet or screenshot showing two or three quotes side by side), return the data from the FIRST quote in the document.
...
- Return ONLY the JSON object, nothing else`
    ...
    try {
      ...
      // "insulation-v3-25cexpired-pricingfix" and store ns "insulation"
      // didn't match — cache lookups never hit.
    ...
    await enrichWithCalibration(redis, parsed, { city: _calCity, state: _calState, service: "insulation" });

    if (req.headers["x-woogoro-test"] !== "1") captureAnonymizedData("insulation", parsed);
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        await guardedFlywheelBump(redis, "insulation", totalPrice, cityLc, st, { incRealQuote: !!_imageBuf });
      }
    } catch (calErr) {
