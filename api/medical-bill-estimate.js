...
- CREDIT BALANCE — ALWAYS apply when the patient-owed line itself is negative (this is the most important rule, do not ignore it): If the bill has a labeled "Patient Responsibility" / "Amount Due" / "Remaining Amount Due" / "Balance Due" / "Total Outstanding" line AND that specific line's number is wrapped in parentheses (e.g., "AMOUNT DUE: ($27.00)") OR has a leading minus sign (e.g., "Balance Due: -$50.00"), the patient has OVERPAID. In that case set patientResponsibility to 0 AND add a redFlag like "Credit balance: provider owes patient a refund of $X — call billing to request a refund check or apply to next visit". NEVER report a positive patientResponsibility value when the bill's patient-owed line is in parentheses or negative — that would tell the patient to pay money they don't owe. Worked example: a bill showing "Total Charges: $535.00" and "Remaining Amount Due: ($27.00)" → patientResponsibility MUST be 0 (not 27, not 535), and add the credit-balance redFlag with refundAmount of $27.
- DO NOT infer a credit balance from negative payment/adjustment lines alone. Negative numbers on "Insurance Payment", "Adjustment", "Contractual Write-off", "Discount" lines are NORMAL accounting (subtractions from charges) and do NOT mean the patient overpaid. If the bill explicitly shows "PATIENT RESPONSIBILITY $390.00" as a positive number, USE that positive number — even if other payment/adjustment lines are negative. Worked example: a bill showing "Insurance payment -$650.00", "Adjustment -$185.00", "PATIENT RESPONSIBILITY $390.00" → patientResponsibility MUST be 390 (the explicit positive number wins over computed inference).
- EXPLICIT-LINE PRIORITY (most important rule for patientResponsibility): If the bill has ANY of these clearly-labeled lines with a visible POSITIVE dollar amount — "PATIENT RESPONSIBILITY", "You Owe", "Amount Due", "Total Outstanding", "Outstanding Balance", "Remaining Balance", "Patient Owes" — use that EXACT amount as patientResponsibility. Do NOT compute, derive, or override it. The patient-resp line is the bill issuer's declaration of what the patient owes; trust it. Only set patientResponsibility=0 when one of those same labeled lines is in parentheses or has a leading minus sign (the credit-balance case).
- DO NOT DOUBLE-COUNT: insurancePaid and adjustments must come from DIFFERENT line items on the bill. If you see one "Adjustment -$2,012" line, that goes in ONE field (typically adjustments), not in BOTH insurancePaid AND adjustments. Reading the same number twice into two fields breaks downstream math.
