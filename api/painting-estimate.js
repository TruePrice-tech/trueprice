...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "painting", cacheNamespace: "painting:v5-multiquote-2026-09-11", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
{
  "contractor": <string or null - painting contractor company name. Look at the top of the quote (letterhead), header, footer, signature line, or "from"/"prepared by" sections. Extract the company name even if it's in a logo image. This is REQUIRED whenever a name is visible.>,
...
- If the document contains MORE THAN ONE distinct painting contractor's quote (e.g. a comparison sheet or screenshot showing two or three quotes side by side), return the data from the FIRST quote in the document.
