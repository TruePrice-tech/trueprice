...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "plumbing", cacheNamespace: "plumbing:v3-l6-2026-05-03", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
- Return ONLY the JSON object, nothing else`
    ...
    try {
      ...
      // cross-vertical L6 finding). Pre-fix lookup ns "plumbing-v2-nullbetter"
      // and store ns "plumbing" didn't match either.
    ...
    if (_guard.imageHash) {
      await storeImageCache(
        "plumbing:v3-l6-2026-05-03",
        _guard.imageHash,
        { success: true, source: "claude-haiku", data: parsed }
      );
    }

    await enrichWithCalibration(redis, parsed, { city: _calCity, state: _calState, service: "plumbing" });

    if (req.headers["x-woogoro-test"] !== "1") captureAnonymizedData("plumbing", parsed); // fire and forget
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        await guardedFlywheelBump(redis, "plumbing", totalPrice, cityLc, st, { incRealQuote: !!_imageBuf });
      }
    } catch (calErr) {
