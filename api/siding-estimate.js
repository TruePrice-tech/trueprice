...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "siding", cacheNamespace: "siding:v5-multiquote-2026-09-11", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
{
  "contractor": <string or null - siding contractor company name. Look at the top of the quote (letterhead), header, footer, signature line, or "from"/"prepared by" sections. Extract the company name even if it's in a logo image. This is REQUIRED whenever a name is visible.>,
...
- If the document contains MORE THAN ONE distinct siding contractor's quote (e.g. a comparison sheet or screenshot showing two or three quotes side by side), return the data from the FIRST quote in the document.
...
- Return ONLY the JSON object, nothing else`
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        await guardedFlywheelBump(redis, "siding", totalPrice, cityLc, st, { incRealQuote: !!_imageBuf });
      }
    } catch (calErr) {
      console.log("[siding-estimate] flywheel bridge error:", calErr.message);
    }
