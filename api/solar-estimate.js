...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "solar", cacheNamespace: "solar:v5-multiquote-2026-09-11", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
- If the document contains MORE THAN ONE distinct solar installer's quote (e.g. a comparison sheet or screenshot showing two or three quotes side by side), return the data from the FIRST quote in the document.
...
- Return ONLY the JSON object, nothing else`
    ...
    if (req.headers["x-woogoro-test"] !== "1") captureAnonymizedData("solar", parsed);
    ...
    try {
      ...
      if (totalPrice > 0 && !_isTestMode) {
        ...
        await guardedFlywheelBump(redis, "solar", totalPrice, cityLc, st, { incRealQuote: !!_imageBuf });
      }
    } catch (calErr) {
