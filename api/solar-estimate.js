...
export default async function handler(req, res) {
  ...
  if (!(await checkRateLimit(clientIp))) {
    ...
    const _imageBuf = (req.body && req.body.images && req.body.images[0])
      ...
      : null;
    const _guard = await runAbuseGuard(req, { vertical: "solar", cacheNamespace: "solar:v2", imageBytes: _imageBuf });
    if (!_guard.ok) {
...
- Return ONLY the JSON object, nothing else`
    ...
    if (_guard.imageHash) {
      await storeImageCache("solar:v2", _guard.imageHash, { success: true, source: "claude-haiku", data: parsed });
    }
