...
## Current state (updated 2026-05-02 — session 7)
...
| 16–18 | (3 more: legal, medical, moving) | ⏳ | — | — | — | — |

**Phase A.2 status: 15 of 18 verticals shipped (roof + hvac + plumbing + electrical + siding + gutter + fencing + painting + landscaping + insulation + concrete + foundation + garage-door + window + auto-repair).** Session 7 added 4 more clean greenfield ships — no pre-existing state hubs to rewrite for any of the 4. Per-state data dicts now total 15 × 50 = 750+ hand-curated state entries with unique distinctive_law and climate_concern strings each, anchored in BLS OEWS May 2024 wages, USDA-NRCS frost lines, NOAA precipitation/lightning data, ASCE 7-22 wind/snow tiers, EPA RRP / SCAQMD / OTC VOC tiers, ACI 318 / ACI 332 / ACI 201.2R concrete classifications, IECC 2021 prescriptive R-value mandatory minimums, IRA HOMES + HEER program implementation status, USDA hardiness zones, ENERGY STAR climate zones (Northern / North-Central / South-Central / Southern), state safety + emissions inspection mandates, state Right-to-Repair (R2R) statute status (MA 2013/2020, ME 2023, CO 2024 + 12 pending), CARB Clean Cars II §177 ZEV state list, EPA radon zone mapping, and state contractor licensing boards verified by URL.
...
**Remaining 3 verticals for Phase A.2 (final batch for next session):**
- legal, medical, moving — all three have ~48 city pages each (similar tiny-NF-baseline situation as auto-repair). Expect similar 3-5pt NF dilution from small baseline; composite stays well above ≥80% floor. Lane has approved this dilution pattern as structural, not a regression.
