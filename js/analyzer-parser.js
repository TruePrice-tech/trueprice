...
export function detectContractor(text) {
  const source = String(text || "");
  const patterns = [
...
export function detectRoofSize(text) {
  const normalized = String(text || "").toLowerCase();
  const candidates = [];
  let match;
  ...
  const explicitPatterns = [
    {
      regex: /\broof size\b[^0-9]{0,25}([0-9]{1,3}(?:,[0-9]{3})?)(?:\.[0-9]+)?\s*(?:sq\.?\s*ft|square feet|sq ft|sf)\b/g,
      source: "roof size label",
      score: 130
    },
    {
      regex: /\broof area\b[^0-9]{0,25}([0-9]{1,3}(?:,[0-9]{3})?)(?:\.[0-9]+)?\s*(?:sq\.?\s*ft|square feet|sq ft|sf)?\b/g,
      source: "roof area label",
      score: 120
    },
    {
      regex: /\btotal roof area\b[^0-9]{0,25}([0-9]{1,3}(?:,[0-9]{3})?)(?:\.[0-9]+)?\s*(?:sq\.?\s*ft|square feet|sq ft|sf)?\b/g,
  ...
  const sqFtRegex = /\b([0-9]{1,3}(?:,[0-9]{3})?)(?:\.[0-9]+)?\s*(?:sq\.?\s*ft|square feet|square foot|sq ft|sf)\b/g;
  while ((match = sqFtRegex.exec(normalized)) !== null) {
    ...
    if (value >= 600 && value <= 12000) {
      ...
      if (/roof size|roof area|total roof area|property info|type of roof|material/.test(context)) score += 20;
      if (/house|living|garage|lot/.test(context)) score -= 20;
      candidates.push({ value, source: "square feet", score });
    }
  }

  const decimalSquaresRegex = /\b([0-9]{1,3}(?:\.[0-9]{1,2})?)\s*(?:squares|square|sq)\b/g;
  while ((match = decimalSquaresRegex.exec(normalized)) !== null) {
  ...
  const roofLine = normalized.match(/roof[^0-9]{0,20}([0-9]{1,3}(?:,[0-9]{3})?)/);
  if (roofLine) {
    const value = Number(String(roofLine[1]).replace(/,/g, ""));
    if (value >= 600 && value <= 12000) {
      candidates.push({ value, source: "roof line fallback", score: 70 });
    }
  }

const labeledRoofSize = normalized.match(
  /\broof size\b[^0-9]{0,20}([0-9]{1,3}(?:,[0-9]{3})?)\s*(?:sq\.?\s*ft|square feet|sq ft|sf)?/i
);

if (labeledRoofSize) {
  const value = Number(String(labeledRoofSize[1]).replace(/,/g, ""));
  if (value >= 600 && value <= 12000) {
    candidates.push({ value, source: "roof size labeled fallback", score: 140 });
  }
}
...
export function detectScopeSignals(text) {
  const results = {};
...
export function detectPremiumSignals(text, signals, roofSize, material) {
  const lower = String(text || "").toLowerCase();
  const items = [];
...
export function detectTotalLinePrice(text) {
  const source = String(text || "");
  ...
  const moneyRegex = /\$?\s*([0-9]{1,3}(?:,[0-9]{3})+(?:\.[0-9]{2})?|[0-9]{4,6}(?:\.[0-9]{2})?)/g;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const matches = [...nearbyBlock.matchAll(moneyRegex)];
    const values = matches
      .map((m) => Number(String(m[1]).replace(/,/g, "")))
      .filter((v) => Number.isFinite(v) && v >= 1000 && v <= 200000);
  ...
  const blockMatch = normalized.match(
    /total(?: estimated)?(?: cost| price| due)?[^0-9$]{0,50}\$?\s*([0-9]{1,3}(?:,[0-9]{3})+|[0-9]{4,6})(?:\.[0-9]{2})?/i
  );

  if (blockMatch) {
    const value = Number(blockMatch[1].replace(/,/g, ""));
...
export function calculateParserConfidence(parsed) {
  let score = 0;
...
export function buildAIExplanation({
  ...
  const materialLabel = materialLabelMap[material] || "roofing";
  const locationLabel = city && stateCode ? `${city}, ${stateCode}` : "your area";

  const lines = [];
  lines.push(
    `This quote appears to be <strong>${verdict}</strong> for a ${roofSize ? formatNumber(roofSize) : "?"} square foot ${materialLabel} project in ${locationLabel}.`
  );

  lines.push(
    `The entered quote is ${formatCurrency(quotePrice)}, while the modeled range is ${formatCurrency(low)} to ${formatCurrency(high)} with a midpoint of ${formatCurrency(mid)}.`
  );

  if (localDataUsed && sizeLabelUsed) {
    lines.push(
      `The pricing model used local benchmark data for a nearby roof size bucket of ${sizeLabelUsed} square feet.`
    );
  } else {
    lines.push(
      `The pricing model used fallback national material benchmarks because exact city pricing data was not available.`
    );
  }

  if (warrantyYears) {
    lines.push(`The quote appears to reference a warranty of about ${warrantyYears} years.`);
  }

  if (tearOffLabel === "yes") {
    lines.push("Tear off appears to be included, which generally increases total cost.");
  } else if (tearOffLabel === "no") {
    lines.push("Tear off appears not to be included, which can make a quote look lower than a full replacement scope.");
  }

  const includedSignals = Object.values(parsedSignals || {})
    .filter((item) => item && item.status === "included")
    .map((item) => item.label);

  if (includedSignals.length) {
    lines.push(`Detected scope items include: ${includedSignals.slice(0, 6).join(", ")}.`);
  }

  if (premiumSignals && premiumSignals.length) {
    lines.push(`Potential premium cost drivers detected: ${premiumSignals.join(", ")}.`);
  }

  if (analysisConfidenceLabel) {
    lines.push(`Parser confidence for this quote is ${analysisConfidenceLabel.toLowerCase()}. Review parsed values before relying on the result.`);
  }
...
export {
  scoreMoneyCandidate,
  normalizeOcrNumberString,
  parsePossiblyBrokenMoney
};
