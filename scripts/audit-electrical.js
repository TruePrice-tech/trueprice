// Electrical thorough audit
const puppeteer = require("puppeteer");
const fs = require("fs");
const path = require("path");

const URL_ANALYZE = "https://woogoro.com/electrical-quote-analyzer.html";
const URL_COMPARE = "https://woogoro.com/compare-electrical-quotes.html";
const URL_ESTIMATE = "https://woogoro.com/electrical-estimate.html";
const FIX_ROOF = path.resolve(__dirname, "..", "test", "receipt", "ocr-cache", "fixtures", "roofing-gaf-quote.jpeg");
const OUT = path.resolve(__dirname, "..", "output", "audits", "electrical-2026-04-29");
...
(async () => {
  ...
  if (STEP === "analyze-reject-roof") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = await page.evaluate(() => /this is not an? electric|looks like.*roof/i.test(document.body.innerText)).catch(() => false);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
    await page.screenshot({ path: path.join(OUT, "analyze/02-roof-on-electrical.png"), fullPage: true });
