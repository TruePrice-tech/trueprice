...
const URL_ANALYZE = "https://woogoro.com/fencing-quote-analyzer.html";
const URL_COMPARE = "https://woogoro.com/compare-fencing-quotes.html";
const URL_ESTIMATE = "https://woogoro.com/fencing-estimate.html";
const FIX_ROOF = path.resolve(__dirname, "..", "test", "receipt", "ocr-cache", "fixtures", "roofing-gaf-quote.jpeg");
const OUT = path.resolve(__dirname, "..", "output", "audits", "fencing-2026-04-29");
...
(async () => {
  ...
  if (STEP === "analyze-reject-roof") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = await page.evaluate(() => /this is not an? fencing|looks like.*roof/i.test(document.body.innerText)).catch(() => false);
