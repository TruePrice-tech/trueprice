// Foundation thorough audit
const puppeteer = require("puppeteer");
const fs = require("fs");
const path = require("path");

const URL_ANALYZE = "https://woogoro.com/foundation-quote-analyzer.html";
const URL_COMPARE = "https://woogoro.com/compare-foundation-quotes.html";
const URL_ESTIMATE = "https://woogoro.com/foundation-estimate.html";
const FIX_ROOF = path.resolve(__dirname, "..", "test", "receipt", "ocr-cache", "fixtures", "roofing-gaf-quote.jpeg");
const OUT = path.resolve(__dirname, "..", "output", "audits", "foundation-2026-04-29");
...
(async () => {
  ...
  if (STEP === "analyze-reject-roof") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = await page.evaluate(() => /this is not.*foundation|looks like.*roof/i.test(document.body.innerText)).catch(() => false);
  ...
  if (STEP === "unhappy-reload") {
    ...
    const stateAfter = await page.evaluate(() => ({
      isInitial: /Is your foundation repair quote fair/i.test(document.body.innerText),
  ...
  if (STEP === "estimate-empty-submit") {
    ...
    await $w(2000);
    await page.evaluate(() => {
      const t = Array.from(document.querySelectorAll("button, input[type=submit]")).find(b => /get\s+foundation.estimate|get\s+estimate/i.test(b.innerText || b.value || ""));
      if (t) t.click();
    });
    await $w(3000);
  ...
  if (STEP === "estimate-wizard") {
    ...
    for (let i = 1; i <= 8; i++) {
      const advanced = await page.evaluate(() => {
        const sels = [".found-option", ".foundation-option", ".gd-option", ".plumb-option", ".hvac-option", "[data-val]", "button.option"];
  ...
  if (STEP === "analyze-reject-hvac") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = /this is not.*foundation|looks like.*hvac/i.test(lastBody);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
  ...
  if (STEP === "analyze-reject-auto") {
    ...
    for (let i = 0; i < 90; i++) {
      await $w(1000);
      const seen = await page.evaluate(() => /this is not.*foundation|looks like.*auto/i.test(document.body.innerText)).catch(() => false);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
  ...
  if (STEP === "step4-all-ctas") {
    ...
    for (const c of cases) {
      ...
      for (let i = 0; i < 200; i++) {
        await $w(1000);
        const seen = await page.evaluate(() => /this is not.*foundation/i.test(document.body.innerText)).catch(() => false);
        if (seen) { rejectSeen = true; break; }
      }
    ...
    const after = await page.evaluate(() => ({
      url: location.href,
      isInitial: /Is your foundation repair quote fair/i.test(document.body.innerText),
      isReject: /this is not.*foundation/i.test(document.body.innerText),
      hasFileInput: !!document.querySelector("input[type=file]"),
  ...
  if (STEP === "diag-ocr-progress") {
    ...
    for (let t = 0; t < 12; t++) {
      ...
      const sample = await page.evaluate(() => ({
        ...
        hasReject: /this is not.*foundation/i.test(document.body.innerText),
  ...
  if (STEP === "step5-unhappy") {
    ...
    log.refreshDuring = await page.evaluate(() => ({
      ...
      hasFileInput: !!document.querySelector("input[type=file]"),
      isInitial: /Is your foundation repair quote fair/i.test(document.body.innerText),
      isStuckOnAnalyzing: /analyzing your foundation|reading text from image/i.test((document.querySelector("h1") || {}).innerText || ""),
    }));
    ...
    for (let i = 0; i < 60; i++) {
      await $w(1000);
      const s = await page.evaluate(() => /this is not.*foundation/i.test(document.body.innerText) || /typical|notable|range|estimated cost/i.test(document.body.innerText)).catch(() => false);
  ...
  if (STEP === "compare-reject-2way") {
    ...
    for (let i = 0; i < 90; i++) {
      await $w(1000);
      try { lastBody = await page.evaluate(() => document.body.innerText.slice(0, 600)); } catch (e) {}
      const seen = /this is not.*foundation|looks like.*roof|looks like.*hvac/i.test(lastBody);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
  ...
  if (STEP === "estimate-step5") {
    ...
    await page.evaluate(() => {
      const els = document.querySelectorAll(".found-option");
    ...
    const after = await page.evaluate(() => ({
      url: location.href,
      h1: (document.querySelector("h1") || {}).innerText || "",
      isInitial: /How much will foundation repair cost|How much does foundation repair cost|foundation repair cost|foundation cost/i.test(document.body.innerText),
  ...
  if (STEP === "compare-step5") {
    ...
    log.refreshDuringParse = await page.evaluate(() => ({
      ...
      isInitial: /Compare your foundation quotes|Compare 2.{0,2}3 foundation/i.test(document.body.innerText),
    }));
