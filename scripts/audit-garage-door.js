...
const URL_ANALYZE = "https://woogoro.com/garage-door-quote-analyzer.html";
const URL_COMPARE = "https://woogoro.com/compare-garage-door-quotes.html";
...
const OUT = path.resolve(__dirname, "..", "output", "audits", "garage-door-2026-04-29");
...
(async () => {
  ...
  if (STEP === "analyze-reject-roof") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = await page.evaluate(() => /this is not an? garage.door|looks like.*roof/i.test(document.body.innerText)).catch(() => false);
  ...
  if (STEP === "reject-ctas") {
    ...
    for (let i = 0; i < 75; i++) {
      await $w(1000);
      const seen = await page.evaluate(() => /this is not.*garage|looks like.*roof/i.test(document.body.innerText)).catch(() => false);
      if (seen) break;
    }
    await $w(2000);
  ...
  if (STEP === "unhappy-reload") {
    ...
    const stateAfter = await page.evaluate(() => ({
      isInitial: /Is your garage door quote fair/i.test(document.body.innerText),
  ...
  if (STEP === "estimate-wizard") {
    ...
    for (let i = 1; i <= 8; i++) {
      const advanced = await page.evaluate(() => {
        const sels = [".gd-option", ".garage-door-option", ".plumb-option", ".hvac-option", "[data-val]", "button.option"];
  ...
  if (STEP === "analyze-reject-hvac") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = /this is not.*garage|looks like.*hvac/i.test(lastBody);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
  ...
  if (STEP === "analyze-reject-auto") {
    ...
    for (let i = 0; i < 90; i++) {
      await $w(1000);
      const seen = await page.evaluate(() => /this is not.*garage|looks like.*auto/i.test(document.body.innerText)).catch(() => false);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
  ...
  if (STEP === "step4-all-ctas") {
    ...
    for (const c of cases) {
      ...
      for (let i = 0; i < 200; i++) {
        await $w(1000);
        const seen = await page.evaluate(() => /this is not.*garage/i.test(document.body.innerText)).catch(() => false);
        if (seen) { rejectSeen = true; break; }
      }
    ...
    const after = await page.evaluate(() => ({
      url: location.href,
      isInitial: /Is your garage door quote fair/i.test(document.body.innerText),
      isReject: /this is not.*garage/i.test(document.body.innerText),
      hasFileInput: !!document.querySelector("input[type=file]"),
  ...
  if (STEP === "diag-ocr-progress") {
    ...
    for (let t = 0; t < 12; t++) {
      ...
      const sample = await page.evaluate(() => ({
        ...
        hasReject: /this is not.*garage/i.test(document.body.innerText),
  ...
  if (STEP === "step5-unhappy") {
    ...
    log.refreshDuring = await page.evaluate(() => ({
      ...
      hasFileInput: !!document.querySelector("input[type=file]"),
      isInitial: /Is your garage door quote fair/i.test(document.body.innerText),
      isStuckOnAnalyzing: /analyzing your garage door|reading text from image/i.test((document.querySelector("h1") || {}).innerText || ""),
    }));
    ...
    for (let i = 0; i < 60; i++) {
      await $w(1000);
      const s = await page.evaluate(() => /this is not.*garage/i.test(document.body.innerText) || /typical|notable|range|estimated cost/i.test(document.body.innerText)).catch(() => false);
  ...
  if (STEP === "compare-reject-2way") {
    ...
    for (let i = 0; i < 90; i++) {
      await $w(1000);
      try { lastBody = await page.evaluate(() => document.body.innerText.slice(0, 600)); } catch (e) {}
      const seen = /this is not.*garage|looks like.*roof|looks like.*hvac/i.test(lastBody);
      if (seen) { rejectSeen = true; break; }
    }
    await $w(2000);
  ...
  if (STEP === "estimate-step5") {
    ...
    const after = await page.evaluate(() => ({
      url: location.href,
      h1: (document.querySelector("h1") || {}).innerText || "",
      isInitial: /How much will your garage door cost/i.test(document.body.innerText),
  ...
  if (STEP === "compare-step5") {
    ...
    log.refreshDuringParse = await page.evaluate(() => ({
      ...
      isInitial: /Compare your garage door quotes/i.test(document.body.innerText),
    }));
