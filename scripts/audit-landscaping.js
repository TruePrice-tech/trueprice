...
const URL_ANALYZE = "https://woogoro.com/landscaping-quote-analyzer.html";
const URL_COMPARE = "https://woogoro.com/compare-landscaping-quotes.html";
const URL_ESTIMATE = "https://woogoro.com/landscaping-estimate.html";
const FIX_ROOF = path.resolve(__dirname, "..", "test", "receipt", "ocr-cache", "fixtures", "roofing-gaf-quote.jpeg");
const OUT = path.resolve(__dirname, "..", "output", "audits", "landscaping-2026-04-29");
...
(async () => {
  ...
  if (STEP === "analyze-reject-roof") {
    ...
    for (let i = 0; i < 90; i++) {
      ...
      const seen = await page.evaluate(() => /this is not an? landscaping|looks like.*roof/i.test(document.body.innerText)).catch(() => false);
