# Backlink Outreach Email Templates

These are link-acquisition pitches (asking someone to ADD a link to YOUR site in their existing content). The widget-embed templates in outreach-emails.md are a separate, complementary play.

Personalize every email. Generic templates get filtered to spam. Always reference a SPECIFIC article you read.

---

## Template 1: Resource Page Add (Highest Hit Rate)

Find pages titled "best home cost calculators", "useful homeowner resources", "links for patients", etc. Then ask to be added.

Search operators that find these:
- `inurl:resources "home improvement"`
- `inurl:links "cost calculator"`
- `intitle:"resources" "home services"`

**Subject:** Quick suggestion for your [page topic] resource list

Hi [Name],

I was reading your [exact page title] and noticed you link to [specific resource on their list, e.g. HomeAdvisor's True Cost Guide]. Useful list.

I run Woogoro (https://woogoro.com), a free homeowner tool that breaks down local contractor pricing by city, material, and contractor tier. Unlike most cost-guide sites, we show ranges and let visitors paste their actual quote for a free analysis -- no email required, no lead-gen sales calls.

Might be a fit for that resource list if you ever update it. Specifically, this page has the same format as the others you link to: https://woogoro.com/guides.html

Either way, thanks for keeping that page maintained. It's a genuinely useful list.

Lane Adams
Woogoro

---

## Template 2: Skyscraper / Better Resource Pitch

Use when you have a deeper or fresher version of an article they linked to.

**Subject:** A more detailed version of [topic] you linked to

Hi [Name],

In your post on [their article title], you linked to [old/thin source] for the cost breakdown section. I noticed that source hasn't been updated since [year], and the numbers are off by ~20-30% in most metros now.

I just published a 2026 cost guide for [topic] that includes:
- Live pricing for 700+ US cities (not national averages)
- Material-tier breakdowns (good/better/best with $ ranges)
- A free quote analyzer if readers want to check their own quote

Link: https://woogoro.com/[specific-guide].html

If you're refreshing the article, would you consider swapping or adding ours? No pressure -- I know link edits are work. Just thought it'd be useful for your readers either way.

Lane
Woogoro

---

## Template 3: Broken Link Replacement

Find a broken outbound link on their site, point it out, suggest yours as the replacement.

Tools to find broken links: Ahrefs Broken Link Checker (free trial), or Chrome ext "Check My Links".

**Subject:** Broken link on [page title]

Hi [Name],

Quick heads up -- the link to [old domain] in your [page title] (in the section about [topic]) returns a 404 now. Looks like that site shut down in [year].

If you're updating, we have a similar resource you could link instead: https://woogoro.com/[specific-guide].html. It covers [the same topic with brief specifics].

No worries either way -- just figured you'd want to know about the dead link.

Lane
Woogoro

---

## Template 4: Journalist / Data Pitch (HARO-style)

For local news, personal finance reporters, real estate columnists. Lead with a unique data point, not a link request.

**Subject:** Data: [Specific city] [service] costs are up [X]% YoY

Hi [Name],

I saw your [recent article] on [topic]. Thought you might find this useful for a future story.

We track real contractor quotes across 700+ US cities. A few interesting data points from the last 90 days:

- [Stat 1, e.g. "Roof replacement quotes in Phoenix are up 18% YoY -- driven by labor shortages and storm damage from the March hailstorms"]
- [Stat 2, e.g. "HVAC install quotes in Dallas have a 2.3x spread between low and high bids -- the widest of any major metro"]
- [Stat 3, with city-specific angle]

Happy to pull more granular data if you ever want a quote-driven angle for a story. Methodology page: https://woogoro.com/methodology.html

Lane
Woogoro

---

## Template 5: Local Real Estate Agent / Mortgage Broker

Real estate agents constantly get asked "how much will it cost to fix X?". A free pricing tool is a value-add for their buyer/seller blog.

**Subject:** Free home cost lookup for your buyer guide

Hi [Name],

I work on Woogoro -- a free tool that shows local home repair and renovation pricing by city. Buyers and sellers in your market ([city]) can look up what a roof, HVAC system, or kitchen remodel typically runs without sitting through a contractor sales call.

If your blog has a "buyer's guide" or "what to know before buying" type page, our [city-specific] cost page might be a useful add-on link for clients trying to budget post-purchase work:

https://woogoro.com/[city]-[state]-roof-cost.html (and similar for HVAC, plumbing, etc.)

No referral fees, no signups, no ads. Just thought it might be useful for your buyers.

Lane
Woogoro

---

## Template 6: Library / .edu / Extension Office (Hardest, Highest Value)

University extension offices, public library research guides, consumer protection orgs. .edu/.gov backlinks are SEO gold.

**Subject:** Free home cost research tool for [library/extension] consumer guides

Hi [Name],

I noticed [library/extension org] maintains a consumer resource page on home repair and renovation budgeting. I wanted to share a free, no-signup tool that may be useful for residents:

https://woogoro.com -- shows local pricing for roofing, HVAC, plumbing, and 17 other home services across 700+ US cities. No email required, no lead-gen, no advertising. Methodology is published openly: https://woogoro.com/methodology.html

The site is run independently (not by a contractor or lead-gen company). Happy to answer any questions about data sourcing if it helps your evaluation.

Lane Adams
Woogoro

---

## Quick Tips

**Send rate:** 5-10 personalized emails/day max. Mass-send and you torch your sender reputation.

**Follow-up:** ONE follow-up after 7 days. "Just bumping this in case it got buried." Then drop it.

**Subject lines:** Lowercase, conversational, no clickbait. "quick question" outperforms "AMAZING resource for you!!!".

**Sign-off:** Use a real name + signature. No corporate template. People link to people, not brands.

**Track:** Keep a CSV of who/when/response. Avoid double-emailing the same domain.

**Use what you already have:** outreach-targets.json has 22 vetted targets. Run these scripts against that list first.
