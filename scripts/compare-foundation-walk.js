// Walk compare-foundation-quotes.html: low + high fixtures.
// CMP-FND-1+2+3 verification.
...
const FIXTURES = [
  path.join(ROOT, "test-quotes/foundation-images/comparison-pier-low.png"),
  path.join(ROOT, "test-quotes/foundation-images/comparison-pier-high.png"),
];
const OUT = "c:/tmp/compare-foundation-walk";
...
(async () => {
  ...
  page.on("response", async res => {
    if (res.url().includes("/api/foundation-estimate") || res.url().includes("/api/parse-quote")) {
      let body = "";
  ...
  console.log("→ navigating compare-foundation-quotes.html");
  ...
  apiResponses.forEach((r, i) => {
    ...
    if (parsed?.data) {
      const d = parsed.data;
      console.log(`api[${i}] status=${r.status} totalPrice=${d.totalPrice} repairMethod=${d.repairMethod} numPiers=${d.numPiers} repairType=${d.repairType} engineerReport=${d.engineerReport} laborTotal=${d.laborTotal} partsTotal=${d.partsTotal} warrantyType=${d.warrantyType} transferable=${d.transferable} contractor=${d.contractor}`);
    } else {
