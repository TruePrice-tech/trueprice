...
function loadCityData() {
  ...
  // The on-disk JSON is an array of city records ({city, state, state_code, ...}).
  // The downstream code expects a dict keyed by `${slug(city)}-${state_code.lower()}`.
  // Without this conversion the script writes files named "0-roof-cost.html",
  // "1-roof-cost.html", etc. (using array indices as keys).
  if (Array.isArray(data)) {
    const dict = {};
    for (const c of data) {
      if (!c || !c.city || !c.state_code) continue;
      const slug = String(c.city).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
      const key = `${slug}-${c.state_code.toLowerCase()}`;
      dict[key] = { city: c.city, state: c.state_code, sizes: c.sizes };
    }
    return dict;
  }
