...
if (!cached.length) {
  console.log("No cached OCR files. Run: node test/electrical/ocr-cache/refresh.js");
  process.exit(0);
}
...
for (const txt of cached) {
  ...
  try {
    const r = parser.parseExtractedTextMultiStrategy(text, "electrical");
...
console.log("── Layer 2 — electrical Real OCR Cache Tests ──");
