...
if (!cached.length) {
  console.log("No cached OCR files. Run: node test/foundation/ocr-cache/refresh.js");
  process.exit(0);
}
...
for (const txt of cached) {
  ...
  try {
    const r = parser.parseExtractedTextMultiStrategy(text, "foundation");
...
console.log("── Layer 2 — foundation Real OCR Cache Tests ──");
