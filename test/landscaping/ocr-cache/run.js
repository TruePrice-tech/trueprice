...
if (!cached.length) {
  console.log("No cached OCR files. Run: node test/landscaping/ocr-cache/refresh.js");
  process.exit(0);
}
...
for (const txt of cached) {
  ...
  try {
    const r = parser.parseExtractedTextMultiStrategy(text, "landscaping");
...
console.log("── Layer 2 — landscaping Real OCR Cache Tests ──");
