...
if (!cached.length) {
  console.log("No cached OCR files. Run: node test/moving/ocr-cache/refresh.js");
  process.exit(0);
}
...
for (const txt of cached) {
  ...
  try {
    const r = parser.parseExtractedTextMultiStrategy(text, "moving");
...
console.log("── Layer 2 — moving Real OCR Cache Tests ──");
