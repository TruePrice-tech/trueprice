#!/usr/bin/env node
// Layer 1 — parser unit tests. <1 second. No browser, no network.
// Feeds hand-curated OCR text snippets directly to parseExtractedTextMultiStrategy.
...
for (const name of snippets) {
  ...
  rows.push({ status, name, expected: exp ? exp.price : null, actual, conf, agreed, minConf: exp ? exp.minConfidence : null });
}

const elapsed = Date.now() - t0;

console.log("── Layer 1 — Parser Unit Tests ──");
const W = { s: 8, n: 34, e: 10, a: 10, c: 10, ag: 5 };
...
for (const r of rows) {
  ...
  const c = r.conf || "—";
  console.log(`${r.status.padEnd(W.s)} ${r.name.padEnd(W.n)} ${e.padEnd(W.e)} ${String(a).padEnd(W.a)} ${c.padEnd(W.c)} ${r.agreed ?? "—"}`);
}
