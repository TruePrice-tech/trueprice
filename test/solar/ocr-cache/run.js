...
if (!cached.length) {
  console.log("No cached OCR files. Run: node test/solar/ocr-cache/refresh.js");
  process.exit(0);
}
...
for (const txt of cached) {
  ...
  try {
    const r = parser.parseExtractedTextMultiStrategy(text, "solar");
...
console.log("── Layer 2 — solar Real OCR Cache Tests ──");
